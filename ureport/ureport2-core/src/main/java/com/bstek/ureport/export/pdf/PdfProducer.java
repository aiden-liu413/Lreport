...
public class PdfProducer implements Producer {
					...
					for(int i=0;i<ps;i++){
						...
						for(Row row:rows){
							...
							for(Column col:columns){
								...
								PdfPCell pdfcell=buildPdfPCell(cell,cellHeight,report);
	...
	private PdfPCell buildPdfPCell(Cell cellInfo,int cellHeight,Report report) throws Exception{
		...
		PdfPCell cell=newPdfCell(cellInfo,cellHeight,report);
		cell.setPadding(0);
	...
	private PdfPCell newPdfCell(Cell cellInfo,int cellHeight,Report report) throws Exception{
		PdfPCell cell=null;
		...
		if(cellData instanceof Image){
			...
			String base64Data = "";
			Map<String, Object> parameters = report.getContext().getParameters();
			String taskId = (String) parameters.get("taskId");
			if(org.apache.commons.lang3.StringUtils.isNotBlank(taskId)){
				base64Data=chartData.retriveBase64Data(taskId);
			}else{
				base64Data=chartData.retriveBase64Data();
			}
			if(base64Data!=null){
