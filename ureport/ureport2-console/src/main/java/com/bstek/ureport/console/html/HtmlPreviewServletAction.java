...
public class HtmlPreviewServletAction extends RenderPageServletAction {
	...
	@Autowired
	ITaskParamProvider taskParamProvider;
	@Override
	...
	public void loadPrintPages(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(org.apache.commons.lang3.StringUtils.isNotBlank(taskId)){
			if(null != taskParamProvider){
				Map<String, Object> paramsByTaskId = taskParamProvider.getParamsByTaskId(taskId);
				paramsByTaskId.putAll(parameters);//开始结束时间使用url带进来的参数
				parameters = paramsByTaskId;
			}
		}
