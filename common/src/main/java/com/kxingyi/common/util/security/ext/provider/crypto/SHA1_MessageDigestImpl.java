...
public class SHA1_MessageDigestImpl extends MessageDigestSpi
	 ...
	 *
	 * @return byte array containing message digest value
	 */
	 ...
	 *
	 * @param buf    byte array to store a message digest returned
	 * @param offset a position in the array for first byte of the message digest
	 * @param len    number of bytes within buffer allotted for the message digest;
	 *               as this implementation doesn't provide partial digests,
	 *               len should be >= 20, DigestException is thrown otherwise
	 * @return the length of the message digest stored in the "buf" buffer;
	 * in this implementation the length=20
	 * @throws IllegalArgumentException       if null is passed to the "buf" argument <BR>
	 *                                        if offset + len > buf.length  <BR>
	 *                                        if offset > buf.length or len > buf.length
	 * @throws DigestException                if len < 20
	 * @throws ArrayIndexOutOfBoundsException if offset < 0
	 */
	 ...
	 *
	 * @return total length of current message digest as an int value
	 */
	 ...
	 *
	 * @param input byte to add to current message
	 */
	 ...
	 * @param offset a number of first byte in the "input" array to use for updating
	 * @param len    a number of bytes to use
	 * @throws NullPointerException           if null is passed to the "buf" argument
	 * @throws IllegalArgumentException       if offset > buf.length or len > buf.length or
	 *                                        (len + offset) > buf.length
	 * @throws ArrayIndexOutOfBoundsException offset < 0
	 */
